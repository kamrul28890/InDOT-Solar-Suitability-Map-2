# Independent R/sf audit of source shapefiles, derived coordinates, attribute
# completeness, geometry validity, score behavior, and SPR_ID overlap.
...
`%||%` <- function(a, b) {
  # Small null-coalescing helper used for command-line and CRS fallbacks.
  if (is.null(a)) b else a
}

# Resolve paths relative to this script so the audit is independent of cwd.
...
markdown_escape <- function(x) {
  # Keep sampled source values from breaking generated Markdown tables.
  x <- gsub("\\|", "\\\\|", x)
  x <- gsub("\r?\n", " ", x)
  x
}
...
relative_path <- function(path) {
  # Normalize Windows separators before removing the repository root prefix.
...
first_examples <- function(x, n = 3) {
  # Show a bounded sample so inventories remain readable on large datasets.
  values <- x[!is.na(x)]
  if (length(values) == 0) {
    return("")
  }
...
field_type <- function(x) {
  # Preserve compound R classes such as POSIXct/POSIXt in one display value.
...
summarize_dataset <- function(name, path) {
  # Return report text and machine-readable tables from one shared inspection,
  # preventing the Markdown/CSV/JSON outputs from drifting apart.
  if (!file.exists(path)) {
    return(list(
  ...
  # Export attributes with reliable geometry-derived coordinates for independent
  # review without requiring GIS software.
  ...
  for (column in names(attrs)) {
    # Build field inventory rows dynamically because DBF schemas differ by layer.
    values <- attrs[[column]]
  ...
  issue_rows <- list()
  # Record data-quality observations as rows rather than stopping the audit.
  ...
  for (field in intersect(score_fields, names(attrs))) {
    # Constant score fields can indicate an upstream model/export problem.
...
# Write human-readable and structured outputs from the same result objects.
inventory_md <- c(
  "# R Data Inventory",
  "",
...
if (all(c("all_candidate_sites", "facility_scored", "row_scored") %in% names(results))) {
  # Compare project identifiers to expose unscored candidates and duplicate
  # membership across facility and right-of-way result sets.
