...
# Keep package, archive, and PyInstaller paths explicit so cleanup cannot affect
# source data or unrelated release artifacts.
...
if (-not $python) {
  throw "Could not find project Python. Expected .venv under website or the repository root."
}

Set-Location $root

function Invoke-Checked {
  # Convert native tool exit codes into terminating release failures.
  param(
...
# Package only after regenerated data, tests, and the production build pass.
...
# The frontend and processed data remain external package folders; PyInstaller
# freezes only the small local API/static-file server.
...
# Assemble the exact end-user layout expected by Run_INDOT_Map.bat.
