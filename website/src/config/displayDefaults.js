...
// Legacy defaults keep older manifests and locally retained exports readable
// while newer manifests carry explicit display metadata.
export const LEGACY_SCORE_FIELDS = [
  { field: 'sol_s', label: 'Solar score', type: 'number' },
  { field: 'slp_s', label: 'Slope score', type: 'number' },
  { field: 'trn_s', label: 'Transmission access', type: 'number' },
...
function normalizeFieldList(fields, fallback = []) {
  // Accept both historic string arrays and current metadata objects.
...
export function resolveLayerDisplay(layer = {}, index = 0) {
  // Merge manifest values over known-project defaults, then generic fallbacks.
...
export function layerLookup(manifest) {
  // Index metadata supports deterministic palette fallback for unknown layers.
