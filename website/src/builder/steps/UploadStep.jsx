...
export function UploadStep({ onLoaded }) {
  // Parsing stays inside the browser; this component owns only file interaction
  // and transient progress/error state.
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);

  async function handleFiles(files) {
    // Both picker and drag/drop paths feed the same parser and parent callback.
    setBusy(true);
    setError('');
    try {
