...
function featureId(datasetName, sourceProps, index) {
  // Prefer source identifiers so repeated imports produce stable builder keys.
...
export async function normalizeLayer(rawLayer, dataset) {
  ...
  for (const [featureIndex, feature] of rawLayer.geojson.features.entries()) {
    // Repair before deriving points/bounds so all downstream spatial operations
    // consume valid geometry and report which features required escalation.
    ...
    const properties = {};
    // Enforce the public data contract instead of forwarding every DBF column.
    ...
    // Turf's pointOnFeature stays on/in irregular polygons and is suitable for
    // overview markers and sidebar-to-map navigation.
