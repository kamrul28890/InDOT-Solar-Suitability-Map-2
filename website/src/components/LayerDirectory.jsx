...
export function LayerDirectory({ directoryLayers, enabled, selectedSite, onLayerToggle, onSiteSelect }) {
  // Render the pre-grouped search result tree. This component only handles
  // interaction and does not duplicate filtering/grouping business logic.
  return (
                ...
                <details className="tree-group" key={`${layer.name}-${group.name}`}>
                  ...
                  <div className="site-list">
                    {group.features.map(({ feature, key }) => (
                      /* Selection delegates layer enablement and fly-to behavior
                         to App so map/sidebar state changes atomically. */
                      <button
