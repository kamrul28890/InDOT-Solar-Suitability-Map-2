...
# Deterministic paths keep cleanup bounded to known build and release artifacts.
...
function Invoke-Checked {
  # Native command failures do not automatically throw in Windows PowerShell.
  param(
...
function Remove-PathWithRetry {
  # Antivirus scanners can briefly retain PyInstaller files after a build.
  param(
    [Parameter(Mandatory = $true)]
    [string]$PathToRemove
  )
...
# Bundle the editor UI, map UI, and GDAL/PROJ resources needed on clean PCs.
...
# Ship only launchers, instructions, and the frozen server directory.
