...
"""FastAPI transport layer for the active Phase 2 editor backend.

Routes in this module stay intentionally thin: request validation and HTTP
delivery happen here, while geospatial and session behavior lives in service.py.
"""
...
def health() -> dict[str, str]:
    """Report that the local editor API process is available."""
...
def api_browse_folder():
    """Open the local native folder picker."""
...
def api_has_autosave():
    """Return metadata for a recoverable autosave, if one exists."""
...
def api_resume_autosave():
    """Restore the latest autosaved session."""
...
def api_import(payload: FolderImportRequest):
    """Import the configured shapefile datasets into a new session."""
...
def api_get_session(session_id: str):
    """Return summary metadata for an editor session."""
...
def api_save_session(session_id: str, payload: SaveSessionRequest):
    """Copy the session JSON to a maintainer-selected backup location."""
...
def api_get_fields(session_id: str):
    """Return field presentation and validation configuration."""
...
def api_put_fields(session_id: str, layer_name: str, payload: FieldConfigRequest):
    """Update user-controlled field settings for one layer."""
...
def api_reset_fields(session_id: str):
    """Restore default field settings derived from imported data."""
...
def api_get_layer_records(
    ...
    """Return merged, filtered records for the editor table."""
...
def api_patch_cell(session_id: str, layer_name: str, feature_id: str, payload: CellPatch):
    """Store one validated cell edit as a session delta."""
...
def api_revert_feature(session_id: str, layer_name: str, feature_id: str):
    """Revert all edits for a feature."""
...
def api_undo(session_id: str):
    """Undo the latest edit or row-revert action."""
...
def api_validate(session_id: str):
    """Validate all merged records before preview or export."""
...
def api_preview(session_id: str):
    """Materialize the current session into the shared preview directory."""
...
@app.get("/preview/")
def preview_index():
    """Serve the generated preview page or a useful empty-state response."""
...
def preview_data(filename: str):
    """Serve generated preview data files."""
...
def preview_asset(file_path: str):
    """Serve preview assets after enforcing directory containment."""

    path = (PREVIEW_DIR / file_path).resolve()
    preview_root = PREVIEW_DIR.resolve()
    # Canonical containment prevents ``..`` traversal from reading local files.
...
def api_export(session_id: str):
    """Generate the deployment-ready map ZIP."""
...
def api_download(token: str):
    """Download a previously generated export package."""

    return FileResponse(export_file_for_token(token), filename=token, media_type="application/zip")


# In development Vite serves the UI separately. Packaged releases bundle the
# built frontend beside the API executable, so FastAPI owns the root route.
dist_dir = ROOT / "dist"
