...
export function ReferenceMap({ records, selectedRecord }) {
  // Leaflet owns DOM outside React, so refs retain the map instance and the
  // replaceable GeoJSON overlay across component renders.
  const containerRef = useRef(null);
  const mapRef = useRef(null);
  const layerRef = useRef(null);

  useEffect(() => {
    // Initialize the basemap once after its container enters the DOM.
  ...
  useEffect(() => {
    // Rebuild the display-only overlay when filters or selection change.
    ...
    // Frame the selected feature when available; otherwise frame all records.
    const selectedLayer = [];
